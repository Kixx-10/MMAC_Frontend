...
class SubmitControllerProvider extends AsyncNotifier<SubmitResponseModel?>{
  ...
  Future<SubmitResponseModel?>submitApplicationAction(SubmitRequestModel requestData)async{
    state = const AsyncLoading(); // set state to loading when submission starts
    final result = await AsyncValue.guard(() => _repository.submitApplication(requestData));
    state = result; // update state with the result (either data or error)
    return result.value; // return the response model if successful, or null if there was an error

  }
}
final submitControllerProvider = AsyncNotifierProvider<SubmitControllerProvider, SubmitResponseModel?>(
